...
public class CrudRequestRouterTest {
    ...
    @Test
    public void disseminate_WhenOtherRequest_CallsNone() {
        Request request = new CrudRequest<RequestableMedia>(Crud.Create){};
        
        router.disseminate(request);
        
        verify(factoidUse, times(0)).execute(request);
        verify(reviewUse, times(0)).execute(request);
        verify(mediaUse, times(0)).execute(request);
    }
